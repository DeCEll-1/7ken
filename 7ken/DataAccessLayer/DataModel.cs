...
namespace DataAccessLayer
{
    public class DataModel
    {
        ...
        IDbConnection dbConnection = new SqlConnection(ConnectionStrings.conStr);
        public DataModel() { con = new SqlConnection(ConnectionStrings.conStr); cmd = con.CreateCommand(); }
        //tamam
        ...
        public List<Kategoriler> KategoriListele()
        {
            try
            {
                ...
                dbConnection.Open();
                List<Kategoriler> kategoriler = dbConnection.Query<Kategoriler>("SELECT * FROM Kategoriler").ToList();
                return kategoriler;
            }
            catch (Exception)
            {
        ...
        public Kategoriler GetKategori(int ID)
        {
            try
            {
                Kategoriler y = dbConnection.QueryFirst<Kategoriler>("SELECT * FROM Kategoriler WHERE ID = @ID", new { ID });
                return y;
            }
            catch (Exception)
            {
        ...
        public bool KategoriGuncelle(Kategoriler k)
        {
            ...
            {
                dbConnection.Open();
                dbConnection.Execute("UPDATE Kategoriler SET KategoriAdi=@KategoriAdi WHERE ID = @ID", new { k.KategoriAdi, k.ID });
                return true;
            }
            catch (Exception)
            {
        ...
        public List<Makaleler> MakaleListele(int KategoriID)
        {
            try
            {
                dbConnection.Open();
                List<Makaleler> kategoriler = dbConnection.Query<Makaleler>("SELECT mak.YoneticiID,mak.Baslik,mak.ID, mak.KategoriID,mak.Ozet,mak.Tamicerik,mak.ThumbnailAdi,mak.TamResimAdi,mak.YuklemeTarihi,mak.Okundu,mak.IsDeleted,kat.KategoriAdi,yon.Adi,yet.YetkiAdi FROM Makaleler AS mak JOIN Yoneticiler AS yon ON Mak.YoneticiID = yon.ID JOIN Kategoriler AS kat ON Mak.KategoriID = kat.ID JOIN Yetkiler AS yet ON Yon.YetkiID = Yet.ID WHERE mak.KategoriID =@KategoriID", new { KategoriID }).ToList();
                return kategoriler;
            }
            catch (Exception)
            {
